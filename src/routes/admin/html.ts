  ...
  <script>
    ...
    async function fetchModels(forceRefresh = false) {
      ...
      try {
        ...
        const res = await fetch(forceRefresh ? API_BASE + '/models?refresh=true' : API_BASE + '/models');
      ...
      const rows = sortedModels.map(model => {
        ...
        const supportHtml = supportedAccounts.length
          ? '<div class="model-support">' + supportedAccounts.map(account =>
              '<span class="model-account-chip">' + escHtml(account.login || account.id || 'Unknown') +
    ...
    function showModal(show) {
      ...
      if (!show && pollInterval) { clearTimeout(pollInterval); pollInterval = null; }
    }
    ...
    function isAuthModalOpen() {
      return document.getElementById('authModal').classList.contains('active');
    }
    function scheduleAuthPoll(deviceCode, accountType, intervalSeconds) {
      if (!isAuthModalOpen()) return;
      if (pollInterval) clearTimeout(pollInterval);
      pollInterval = setTimeout(() => {
        pollInterval = null;
        pollAuth(deviceCode, accountType);
    ...
    async function startAuth() {
      try {
        ...
        scheduleAuthPoll(data.deviceCode, accountType, currentInterval);
    ...
    async function pollAuth(deviceCode, accountType) {
      try {
        ...
        if (data.success) {
          ...
          scheduleAuthPoll(deviceCode, accountType, currentInterval);
        } else {
          scheduleAuthPoll(deviceCode, accountType, currentInterval);
        }
      } catch (e) {
